...
public class ClassAdvancementDialog extends Sprite
{

    public static const JOB_ADVANCEMENTS = {
        "Ninja": [
                "Super Ninja",
                "Samurai"
        ]
    ...
    public function ClassAdvancementDialog(player: Player)
    {
        const playersJob = player.props_.id_;
        this.box_ = new Sprite();
        super();
        if (!(playersJob in JOB_ADVANCEMENTS)) {
            return;
        }
        const title = "Job Advancement";
        const text = "Advance job?";
        const button1 = JOB_ADVANCEMENTS[playersJob][0];
        const button2 = JOB_ADVANCEMENTS[playersJob][1];
