...
public class EditingScreen extends Sprite {
    ...
    public var copiedTiles_:Vector.<IntPoint>;
    ...
    private function onTilesEvent(event:TilesEvent):void {
        ...
        switch (this.commandMenu_.getCommand()) {
            ...
            case MECommandMenu.COPY_COMMAND:
                if(event.tiles_.length > 1) {
                    copiedTiles_ = event.tiles_;
            ...
            case MECommandMenu.PASTE_COMMAND:
                ...
                for each(var tileMod in tilesToPaste) {
                    var originType = meMap_.getType(tileMod.x_, tileMod.y_, this.chooser_.layer_);
                    tileMod.x_ -= originTile.x_;
                    tileMod.y_ -= originTile.y_;
                    tileMod.x_ += tile.x_;
                    tileMod.y_ += tile.y_;
                    var replaceType = meMap_.getType(tileMod.x_, tileMod.y_, this.chooser_.layer_);
                    if(originType != replaceType) {
                        commandList.addCommand(new MEModifyCommand(this.meMap_,
                                tileMod.x_,
                                tileMod.y_,
                                this.chooser_.layer_,
                                replaceType,
                                originType));
                    }
                }
                if(!commandList.empty()) {
                    commandQueue_.addCommandList(commandList);
                }
                break;
